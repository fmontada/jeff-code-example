...
variable "create_acm" {
  type        = bool
  default     = true
  description = "When set to 'true' the ACM certificate will be created, otherwise will use the one you specify"
}

variable "validate_all_acm_subdomains" {
  type        = bool
  default     = true
  description = "When set to 'true' the ACM certificate will validate *.example.com, otherwise will validate subdomain.example.com. When subdomain.example.com is the first element of aliases"
}

variable "domain_depth" {
  type = number
  default = 2
  description = "Depth to use as primary domain to create the ACM certificate, based on the first element of aliases. Example: if aliases = [subdomain.example.com] and domain_depth is 2 the acm certificate will be valid for `example.com` and aliases"
}
